...
import { IFinalizeExpiredContestsUseCase } from '@/Application/Cron/Interfaces/IFinalizeExpiredContest.usecase';
...
export const CronScheduler = (
    ...
    finalizeContests: IFinalizeExpiredContestsUseCase
) => {
    ...
    cron.schedule('* * * * *', async () => {
        try {
            logger.info('Running Contest Finalization Job...');

            await finalizeContests.execute();

            logger.info('Contest Finalization Job Completed.');
        } catch (error) {
            logger.error(
                { error },
                'Contest Finalization Job Failed'
            );
        }
    });
